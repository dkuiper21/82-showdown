...
// Marginal value a single player adds to a squad at a given slot, using the
// same era factor and bench weighting as ratings() below. A team's scoring
// margin is roughly 0.22*ownOff + 0.095*ownDef, so weighting the two parts the
// same way makes "highest playerValue" line up with "most likely to win".
// Used by the CPU opponent to pick the best available option each round.
export function playerValue(pl, year, slot) {
  const w = slot < 5 ? 1.0 : 0.55; // bench minutes weight
  const f = eraFactor(year);
  const off = w * f * (pl.pts + 1.4 * pl.ast);
  const def = w * (f * 0.8 * pl.reb + 2.8 * pl.stl + 2.8 * pl.blk);
  return 0.22 * off + 0.095 * def;
}
